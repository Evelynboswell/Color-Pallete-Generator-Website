const buildPalette = (colorsList) => {
  ...
  for (let i = 0; i < orderedByColor.length; i++) {
    ...
    if (i > 0) {
      ...
      // if the color is too close to the previous color, skip it
      if (difference < 120) {
        continue;
      }
    }
    ...
    colorElement.style.transition = "background-color 0.3s ease";
    colorElement.addEventListener("click", () => {
      // Create a temporary textarea to hold the hex code
      const tempTextArea = document.createElement("textarea");
      tempTextArea.value = hexColor;
      document.body.appendChild(tempTextArea);
      
      // Select and copy the hex code
      tempTextArea.select();
      document.execCommand("copy");
      
      // Remove the temporary textarea
      document.body.removeChild(tempTextArea);
      // Optionally, provide visual feedback on copy (e.g., changing color background momentarily)
      colorElement.style.backgroundColor = "lightgreen";
      setTimeout(() => {
        colorElement.style.backgroundColor = hexColor;
      }, 300); // Reset background color after 1 second
    ...
    if (hslColors[i].h) {
      ...
      const hexComplementary = hslToHex(hslColors[i]);
    
      complementaryElement.style.cursor = "pointer";
      complementaryElement.style.transition = "background-color 0.3s ease";
      complementaryElement.addEventListener("click", () => {
      // Create a temporary textarea to hold the hex code
      const tempTextArea = document.createElement("textarea");
      tempTextArea.value = hexComplementary;
      document.body.appendChild(tempTextArea);
      ...
      document.body.removeChild(tempTextArea);
      // Optionally, provide visual feedback on copy (e.g., changing color background momentarily)
      complementaryElement.style.backgroundColor = "lightgreen";
      setTimeout(() => {
        complementaryElement.style.backgroundColor = hexComplementary;
      }, 300); // Reset background color after 1 second
      // Copy hex code to clipboard
      navigator.clipboard.writeText(hexComplementary)
      .then(() => {
      // Notify user about successful copy
      alert("Copied to clipboard!");
      })
...
// Not as accurate as Delta-E distance metric.
...
const findBiggestColorRange = (rgbValues) => {
  /**
   1 Min is initialized to the maximum value posible
   2 Find the minimum value for that color channel
   
   1 Max is initialized to the minimum value posible
   2 Find the maximum value for that color channel
   */
...
const main = () => {
  const imgFile = document.getElementById("input-file");
